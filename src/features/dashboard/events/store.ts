...
import type { EditEventErrors, EditEventFormState } from './popups/EditEvent';
import type { NewEventErrors, NewEventFormState } from './popups/NewEvent';
...
type EventStore = {
  ...
  result: {
    isOpened: boolean;
    open: (event: ResultEvent) => void;
    close: () => void;
    event: ResultEvent | undefined;
  };
};
...
export const useEventStore = create<EventStore>()(
  immer(set => ({
    ...
    result: {
      ...
      open: event =>
        set(state => {
          state.result.isOpened = true;
          state.result.event = event;
        }),
      close: () =>
        set(state => {
          state.result.isOpened = false;
          state.result.event = undefined;
        }),
      event: undefined,
    },
  }))
